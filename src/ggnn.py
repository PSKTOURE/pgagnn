"""Equivariant transformer for vector and scalar data."""

import math

import torch
import torch.nn.functional as F
from torch import nn
from torch.utils.checkpoint import checkpoint


def get_nonlinearity(label):
    if label == "relu":
        return nn.ReLU()
    elif label == "sigmoid":
        return nn.Sigmoid()
    elif label == "tanh":
        return nn.Tanh()
    elif label == "gelu":
        return nn.GELU()
    elif label == "silu":
        return nn.SiLU()
    else:
        raise ValueError(f"Unsupported nonlinearity type: {label}")


def _post_attention_reshape(out, hidden_vc):
    h_v = out[..., : hidden_vc * 3].reshape(*out.shape[:-1], hidden_vc, 3)
    h_s = out[..., hidden_vc * 3 :]

    h_v = h_v.movedim(-3, -4).flatten(-3, -2)
    h_s = h_s.movedim(-2, -3).flatten(-2, -1)
    return h_v, h_s


class Dropout(nn.Module):
    def __init__(self, dropout_prob: float):
        super().__init__()
        self._dropout_prob = dropout_prob

    def forward(self, vectors, scalars):
        if not self.training or self._dropout_prob == 0.0:
            return vectors, scalars

        mask_shape = vectors.shape[:-1] + (1,)
        v_mask = F.dropout(
            torch.ones(mask_shape, dtype=vectors.dtype, device=vectors.device),
            p=self._dropout_prob,
            training=self.training,
        )
        out_v = vectors * v_mask

        out_s = F.dropout(scalars, p=self._dropout_prob, training=self.training)

        return out_v, out_s


class RMSNorm(nn.Module):
    """Normalize jointly over vector and scalar features."""

    def __init__(self, epsilon: float = 0.01):
        super().__init__()
        self.epsilon = epsilon

    def forward(self, vectors, scalars):
        v_squared_norm = vectors.square().sum(dim=-1)
        s_squared_norm = scalars.square()
        total_features = v_squared_norm.shape[-1] + s_squared_norm.shape[-1]
        mean_squared_norms = (v_squared_norm.sum(dim=-1) + s_squared_norm.sum(dim=-1)) / total_features
        norm = torch.rsqrt(mean_squared_norms + self.epsilon)
        vectors_out = vectors * norm[..., None, None]
        scalars_out = scalars * norm[..., None]
        return vectors_out, scalars_out


class Linear(nn.Module):
    """Linear operations for vector and scalar features.

    Supports optional mixing between vector and scalar features to improve expressivity.
    """

    def __init__(
        self,
        in_vc: int,
        out_vc: int,
        in_sc: int,
        out_sc: int,
        bias: bool = True,
        initialization: str = "default",
    ):
        """
        Parameters
        ----------
        in_vc : int
            Number of input vector channels.
        out_vc : int
            Number of output vector channels.
        in_sc : int
            Number of input scalar channels.
        out_sc : int
            Number of output scalar channels.
        bias : bool, optional
            Whether to include a bias term in the scalar linear layer, by default True.
        initialization : str, optional
            Initialization method for weights, by default "default".
            The alternative "small" initializes weights to smaller values,
            which might improve stability in attention projections.
        """
        super().__init__()
        self._in_vc = in_vc
        self._out_vc = out_vc
        self._in_sc = in_sc
        self._out_sc = out_sc
        self._bias = bias

        self.weight_v = nn.Parameter(
            torch.empty(
                (
                    out_vc,
                    in_vc,
                )
            )
        )
        self.linear_s = nn.Linear(in_sc, out_sc, bias=bias)

        self.reset_parameters(initialization)

    def forward(self, vectors, scalars):
        """
        Parameters
        ----------
        vectors : torch.Tensor
            A tensor of shape (..., vc, 3) representing vectors.
        scalars : torch.Tensor
            A tensor of shape (..., sc) representing scalar features.

        Returns
        -------
        torch.Tensor, torch.Tensor
            Tensors of the same shape as input representing the normalized vectors and scalars.
        """
        vectors_out = self.weight_v @ vectors
        scalars_out = self.linear_s(scalars)
        return vectors_out, scalars_out

    def reset_parameters(self, initialization, additional_factor=1.0):
        if initialization == "default":
            v_factor = additional_factor
            s_factor = additional_factor
        elif initialization == "small":
            v_factor = 0.1 * additional_factor
            s_factor = 0.1 * additional_factor
        else:
            raise ValueError(f"Unknown initialization: {initialization}")

        fan_in = max(self._in_vc, 1)
        bound = v_factor / math.sqrt(fan_in)
        nn.init.uniform_(self.weight_v, a=-bound, b=bound)

        fan_in = max(self._in_sc, 1)
        bound = s_factor / math.sqrt(fan_in)
        nn.init.uniform_(self.linear_s.weight, a=-bound, b=bound)


class GatedLinearUnit(nn.Module):
    """Gated linear unit (GLU) for vector and scalar features.

    Scalar gates are computed from scalar features,
    while vector gates are computed from inner products of vector features.
    """

    def __init__(
        self,
        in_vc: int,
        out_vc: int,
        in_sc: int,
        out_sc: int,
        nonlinearity: str = "gelu",
    ):
        super().__init__()
        self.linear = Linear(
            in_vc=in_vc,
            out_vc=3 * out_vc,
            in_sc=in_sc,
            out_sc=2 * out_sc,
        )
        self.nonlinearity = get_nonlinearity(nonlinearity)

    def forward(self, vectors, scalars):
        """
        Parameters
        ----------
        vectors : torch.Tensor
            A tensor of shape (..., vc, 3) representing vectors.
        scalars : torch.Tensor
            A tensor of shape (..., sc) representing scalar features.

        Returns
        -------
        torch.Tensor, torch.Tensor
            Tensors of the same shape as input representing the normalized vectors and scalars.
        """
        v_full, s_full = self.linear(vectors, scalars)
        v_pre, v_gates_1, v_gates_2 = v_full.chunk(3, dim=-2)
        s_pre, s_gates = s_full.chunk(2, dim=-1)

        v_gates = (v_gates_1 * v_gates_2).sum(dim=-1, keepdim=True)

        vectors_out = self.nonlinearity(v_gates) * v_pre
        scalars_out = self.nonlinearity(s_gates) * s_pre
        return vectors_out, scalars_out


class EdgeBiasedSelfAttention(nn.Module):
    """Self-attention module with Attention and Value biasing via edge features."""

    def __init__(
        self,
        vc: int,
        sc: int,
        edge_vc: int,  # e.g., 1 if just relative positions
        edge_sc: int,  # e.g., bond type features + distances
        num_heads: int,
        attn_ratio: int = 1,
        dropout_prob: float | None = None,
    ):
        super().__init__()
        self.hidden_vc = max(attn_ratio * vc // num_heads, 1)
        self.hidden_sc = max(attn_ratio * sc // num_heads, 4)
        self.num_heads = num_heads

        # Combined feature dimension per head
        self.d_head = self.hidden_vc * 3 + self.hidden_sc

        # Standard Node Projections
        self.linear_in = Linear(
            in_vc=vc,
            out_vc=3 * self.hidden_vc * self.num_heads,
            in_sc=sc,
            out_sc=3 * self.hidden_sc * self.num_heads,
            initialization="small",
        )
        self.linear_out = Linear(
            in_vc=self.hidden_vc * self.num_heads,
            out_vc=vc,
            in_sc=self.hidden_sc * self.num_heads,
            out_sc=sc,
            initialization="small",
        )
        self.norm = RMSNorm()

        self.edge_to_attn_bias = nn.Linear(edge_sc, num_heads)

        self.edge_v_to_value = nn.Linear(edge_vc, self.hidden_vc * num_heads, bias=False)
        self.edge_s_to_value = nn.Linear(edge_sc, self.hidden_sc * num_heads)

        self.dropout = Dropout(dropout_prob) if dropout_prob is not None else None

    def _pre_attention_reshape(self, qkv_v, qkv_s):
        qkv_v = qkv_v.unflatten(-2, (3, self.hidden_vc, self.num_heads)).movedim(-4, 0).movedim(-2, -4)
        qkv_s = qkv_s.unflatten(-1, (3, self.hidden_sc, self.num_heads)).movedim(-3, 0).movedim(-1, -3)

        q_v, k_v, v_v = qkv_v.unbind(0)
        q_s, k_s, v_s = qkv_s.unbind(0)

        q = torch.cat([q_v.flatten(start_dim=-2), q_s], dim=-1)
        k = torch.cat([k_v.flatten(start_dim=-2), k_s], dim=-1)
        v = torch.cat([v_v.flatten(start_dim=-2), v_s], dim=-1)
        return q, k, v

    def forward(self, vectors, scalars, edge_vectors, edge_scalars, adj, **attn_kwargs):
        """
        Parameters
        ----------
        vectors : (B, N, vc, 3)
        scalars : (B, N, sc)
        edge_vectors : (B, N, N, edge_vc, 3) e.g. relative displacements
        edge_scalars : (B, N, N, edge_sc) e.g. distances, bond types
        adj : (B, N, N) adjacency matrix
        """
        _B, _N, _, _ = vectors.shape

        qkv_v, qkv_s = self.linear_in(vectors, scalars)
        q, k, v = self._pre_attention_reshape(qkv_v, qkv_s)

        attn_logits = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_head)
        attn_logits = attn_logits.masked_fill(adj.unsqueeze(1) == 0, float("-inf"))

        bias = self.edge_to_attn_bias(edge_scalars).permute(0, 3, 1, 2) / math.sqrt(self.hidden_sc)
        attn_logits = attn_logits + bias

        attn_weights = F.softmax(attn_logits, dim=-1)
        node_output = torch.matmul(attn_weights, v)

        edge_v_v = self.edge_v_to_value(edge_vectors.movedim(-2, -1)).movedim(-1, -2)
        edge_v_v = edge_v_v.unflatten(-2, (self.num_heads, self.hidden_vc)).movedim(3, 1)
        edge_v_v_flat = edge_v_v.flatten(start_dim=-2)

        edge_v_s = (
            self.edge_s_to_value(edge_scalars).unflatten(-1, (self.num_heads, self.hidden_sc)).movedim(-2, 1)
        )

        edge_values = torch.cat([edge_v_v_flat, edge_v_s], dim=-1)
        edge_output = torch.einsum("b h i j, b h i j d -> b h i d", attn_weights, edge_values)

        out = node_output + edge_output
        h_v, h_s = _post_attention_reshape(out, self.hidden_vc)
        out_v, out_s = self.linear_out(h_v, h_s)

        if self.dropout is not None:
            out_v, out_s = self.dropout(out_v, out_s)
        return out_v, out_s


class MLP(nn.Module):
    """Multi-layer perceptron (MLP) for vector and scalar features."""

    def __init__(
        self,
        vc: int,
        sc: int,
        nonlinearity: str = "gelu",
        mlp_ratio: int = 2,
        num_layers: int = 2,
        dropout_prob: float | None = None,
    ):
        super().__init__()
        assert num_layers >= 2
        layers = []

        vc_list = [vc] + [mlp_ratio * vc] * (num_layers - 1) + [vc]
        sc_list = [sc] + [mlp_ratio * sc] * (num_layers - 1) + [sc]

        for i in range(num_layers - 1):
            layers.append(
                GatedLinearUnit(
                    in_vc=vc_list[i],
                    out_vc=vc_list[i + 1],
                    in_sc=sc_list[i],
                    out_sc=sc_list[i + 1],
                    nonlinearity=nonlinearity,
                )
            )
            if dropout_prob is not None:
                layers.append(Dropout(dropout_prob))
        layers.append(
            Linear(
                in_vc=vc_list[-2],
                out_vc=vc_list[-1],
                in_sc=sc_list[-2],
                out_sc=sc_list[-1],
            )
        )

        self.layers = nn.ModuleList(layers)

    def forward(self, vectors, scalars):
        """
        Parameters
        ----------
        vectors : torch.Tensor
            A tensor of shape (..., vc, 3) representing vectors.
        scalars : torch.Tensor
            A tensor of shape (..., sc) representing scalar features.

        Returns
        -------
        torch.Tensor, torch.Tensor
            Tensors of the same shape as input representing the normalized vectors and scalars.
        """
        v, s = vectors, scalars

        for layer in self.layers:
            v, s = layer(v, scalars=s)

        return v, s


class GGNN_Block(nn.Module):
    """A single block of the GATr-slim,
    consisting of self-attention and MLP layers, pre-norm and residual connections."""

    def __init__(
        self,
        vc: int,
        sc: int,
        edge_vc: int,
        edge_sc: int,
        num_heads: int,
        nonlinearity: str = "gelu",
        mlp_ratio: int = 2,
        attn_ratio: int = 1,
        num_layers_mlp: int = 2,
        dropout_prob: float | None = None,
    ):
        super().__init__()

        self.norm = RMSNorm()

        self.attention = EdgeBiasedSelfAttention(
            vc=vc,
            sc=sc,
            edge_vc=edge_vc,
            edge_sc=edge_sc,
            num_heads=num_heads,
            attn_ratio=attn_ratio,
            dropout_prob=dropout_prob,
        )

        self.mlp = MLP(
            vc=vc,
            sc=sc,
            nonlinearity=nonlinearity,
            mlp_ratio=mlp_ratio,
            num_layers=num_layers_mlp,
            dropout_prob=dropout_prob,
        )

    def forward(self, vectors, scalars, edge_vectors, edge_scalars, adj, **attn_kwargs):
        """
        Parameters
        ----------
        vectors : torch.Tensor
            A tensor of shape (..., vc, 3) representing vectors.
        scalars : torch.Tensor
            A tensor of shape (..., sc) representing scalar features.
        edge_vectors : torch.Tensor
            A tensor of shape (..., edge_vc, 3) representing edge vectors.
        edge_scalars : torch.Tensor
            A tensor of shape (..., edge_sc) representing edge scalar features.
        adj : torch.Tensor
            A tensor of shape (..., N, N) representing the adjacency matrix.
        **attn_kwargs : dict
            Additional keyword arguments for the attention function.

        Returns
        -------
        torch.Tensor, torch.Tensor
            Tensors of the same shape as input representing the normalized vectors and scalars.
        """
        h_v, h_s = self.norm(vectors, scalars)

        h_v, h_s = self.attention(
            h_v,
            h_s,
            edge_vectors,
            edge_scalars,
            adj,
            **attn_kwargs,
        )

        outputs_v = vectors + h_v
        outputs_s = scalars + h_s

        h_v, h_s = self.norm(outputs_v, outputs_s)

        h_v, h_s = self.mlp(h_v, h_s)

        outputs_v = outputs_v + h_v
        outputs_s = outputs_s + h_s

        return outputs_v, outputs_s


class GGNN(nn.Module):
    """GATr-slim network."""

    def __init__(
        self,
        in_vc: int,
        out_vc: int,
        hidden_vc: int,
        in_sc: int,
        out_sc: int,
        hidden_sc: int,
        edge_vc: int,
        edge_sc: int,
        num_layers: int,
        num_heads: int,
        nonlinearity: str = "gelu",
        mlp_ratio: int = 2,
        attn_ratio: int = 1,
        num_layers_mlp: int = 2,
        dropout_prob: float | None = None,
        checkpoint_blocks: bool = False,
    ):
        """
        Parameters
        ----------
        in_vc : int
            Number of input vector channels.
        out_vc : int
            Number of output vector channels.
        hidden_vc : int
            Number of hidden vector channels.
        in_sc : int
            Number of input scalar channels.
        out_sc : int
            Number of output scalar channels.
        hidden_sc : int
            Number of hidden scalar channels.
        num_layers : int
            Number of GATr Transformer blocks.
        num_heads : int
            Number of attention heads.
        nonlinearity : str, optional
            Nonlinearity type for MLP layers, by default "gelu".
        mlp_ratio : int, optional
            Expansion ratio for MLP hidden layers, by default 2.
        attn_ratio : int, optional
            Expansion ratio for attention hidden layers, by default 1.
        num_layers_mlp : int, optional
            Number of layers in MLP, by default 2.
        dropout_prob : float | None, optional
            Dropout probability, by default None.
        checkpoint_blocks : bool, optional
            Whether to use gradient checkpointing for blocks, by default False.
        compile : bool, optional
            Whether to compile the model with torch.compile, by default False.
        compile_mode : str
            torch.compile compilation mode, see torch docs for more information.
        compile_dynamic : bool
            Whether to use dynamic shapes with torch.compile, by default True.
        """
        super().__init__()

        self.norm = RMSNorm()
        self.linear_in = Linear(
            in_vc=in_vc,
            in_sc=in_sc,
            out_vc=hidden_vc,
            out_sc=hidden_sc,
        )

        self.blocks = nn.ModuleList(
            [
                GGNN_Block(
                    vc=hidden_vc,
                    sc=hidden_sc,
                    edge_vc=edge_vc,
                    edge_sc=edge_sc,
                    num_heads=num_heads,
                    nonlinearity=nonlinearity,
                    mlp_ratio=mlp_ratio,
                    attn_ratio=attn_ratio,
                    num_layers_mlp=num_layers_mlp,
                    dropout_prob=dropout_prob,
                )
                for _ in range(num_layers)
            ]
        )

        self.linear_out = Linear(
            in_vc=hidden_vc,
            in_sc=hidden_sc,
            out_vc=out_vc,
            out_sc=out_sc,
        )
        self._checkpoint_blocks = checkpoint_blocks

    def forward(self, vectors, scalars, edge_vectors, edge_scalars, adj, **attn_kwargs):
        """
        Parameters
        ----------
        vectors : torch.Tensor
            A tensor of shape (..., vc, 3) representing vectors.
        scalars : torch.Tensor
            A tensor of shape (..., sc) representing scalar features.
        edge_vectors : torch.Tensor
            A tensor of shape (..., edge_vc, 3) representing edge vectors.
        edge_scalars : torch.Tensor
            A tensor of shape (..., edge_sc) representing edge scalar features.
        adj : torch.Tensor
            A tensor of shape (..., N, N) representing the adjacency matrix.
        **attn_kwargs : dict
            Additional keyword arguments for the attention function.

        Returns
        -------
        torch.Tensor, torch.Tensor
            Tensors of the same shape as input representing the normalized vectors and scalars.
        """
        h_v, h_s = self.linear_in(vectors, scalars)
        edge_v, edge_s = self.norm(edge_vectors, edge_scalars)
        # add self loops
        adj = torch.maximum(adj, torch.eye(adj.size(-1), device=adj.device).unsqueeze(0))
        for block in self.blocks:
            if self._checkpoint_blocks:
                h_v, h_s = checkpoint(
                    block, h_v, h_s, edge_v, edge_s, adj, use_reentrant=False, **attn_kwargs
                )
            else:
                h_v, h_s = block(h_v, h_s, edge_v, edge_s, adj, **attn_kwargs)

        outputs_v, outputs_s = self.linear_out(h_v, h_s)
        return outputs_v, outputs_s
